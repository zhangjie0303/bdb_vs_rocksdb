...
main(int argc, char *argv[])
{
    ...
    try {
        ...
        std::cout << "before count record." << std::endl;
        int count_records = countRecords(dbp, NULL);
        std::cout << "after count record: " << count_records << std::endl;
...
writerThread(void *args)
{
    ...
    int thread_total_put = 0;
	std::string thread_index_str = std::to_string(thread_num);
    ...
    for (int i=0; i<5000000; i++) {
        ...
        while (retry) {
            ...
            try {
        		...
        		std::string key_str = "foo_" + thread_index_str + "_" + sufix;
        		std::string value_str = "bar_" + thread_index_str + "_" + sufix;

                // Perform the database write for this transaction.
                Dbt key, value;
                key.set_data((void *)key_str.c_str());
                key.set_size((u_int32_t)key_str.length() + 1);

                value.set_data((void *)value_str.c_str());
                value.set_size((u_int32_t)value_str.length() + 1);

                // Perform the database put
                dbp->put(txn, &key, &value, 0);
                ++thread_total_put;
                //char *key_string = (char *)key.get_data();
                //char *value_string = (char *)value.get_data();
                //std::cout << "[" << key_string << ", " << value_string << "]" << std::endl;
                ...
                if (thread_total_put % 1000 == 0) {
                    std::cout << "thread num: " << thread_num << ", " << thread_total_put << std::endl;
                }
                // commit
                try {
                    txn->commit(0);
    ...
    uint64_t cost = get_now_ms() - start_time;
    std::cout << "thread num: " << thread_num << ", total cost: " << cost
        << "ms, avg cost: " << double(cost) / thread_total_put << std::endl;
    return (0);
}
