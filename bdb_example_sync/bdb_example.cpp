...
int openDb(Db **dbpp, const char *progname, const char *fileName, DbEnv *envp,
	...
	try {
		...
		*dbpp = dbp;
		...
		std::cerr << progname
			  << ": openDb: db open failed:" << std::endl;
...
uint64_t countRecords(Db *dbp, DbTxn *txn) {
	...
	if (cursorp != NULL) {
		try {
			...
			std::cerr << "countRecords: cursor close failed:"
				  << std::endl;
			std::cerr << e.what() << std::endl;
		}
	}
...
int main() {
	...
	std::cout << "open cost: " << TimeHelper::micros() - start_0
		  << std::endl;
#else
	int ret = 0;
	int max_retries = 20;  // Max retry on a deadlock
	...
	for (uint64_t i = 0; i < max_write_count; i++) {
		int key_len =
		    sprintf(buf, "abcdefghijklmn_%d_opqrstuvwxyz_key", i);
		buf[key_len] = '\0';
		int data_len =
		    sprintf(rbuf, "berkeley_valu_%d_data_test_value", i);
		rbuf[data_len] = '\0';
		Dbt key(buf, key_len + 1);
		Dbt data(rbuf, data_len + 1);

		// ret = dbp->put(0, &key, &data, DB_NOOVERWRITE);
		ret = dbp->put(0, &key, &data, DB_OVERWRITE_DUP);
		// if (ret == DB_KEYEXIST) {
		//	std::cout << "Key " << buf << " already exists." <<
		//std::endl;
		// }
		if (dbp->sync(0) != 0) {
			std::cout << "dbp sync error." << std::endl;
		}
	...
	std::cout << "write total: " << max_write_count << ", cost: " << cost
		  << "micros, avg: " << double(cost) / max_write_count
		  << "micros" << std::endl;
	std::cout << "----------------------------------- put "
		     "done!----------------------------------------"
		  << std::endl;
	...
	for (uint64_t i = 0; i < random_read_count; i++) {
		...
		try {
			...
			Dbc *dbcp;
			int key_len =
			    sprintf(buf, "abcdefghijklmn_%d_opqrstuvwxyz_key",
	...
	std::cout << "read total: " << random_read_count
		  << ", cost: " << cost_read_micros << "micros, avg: "
		  << double(cost_read_micros) / random_read_count << "micros"
		  << std::endl;
	std::cout << "----------------------------------- read "
		     "done!----------------------------------------"
		  << std::endl;
// scan
	...
	try {
		...
		std::cout << "key: " << (char *)key.get_data()
			  ...
			  << ", get cost: "
			  << TimeHelper::micros() - start_scan_micros
			  << "micros" << std::endl;
	...
	uint64_t total_count = countRecords(dbp, NULL);
	std::cout << "count total cost: "
		  << TimeHelper::micros() - start_count_micros
		  << "micros, total count: " << total_count << std::endl;
	std::cout << "----------------------------------- count "
		     "done!----------------------------------------"
		  << std::endl;
